  ...
  const handleSubmit = async () => {
    try {
      const itemData = {
        ...
        time: formData.date ? new Date(formData.date).toISOString() : new Date().toISOString(),
        image: formData.images[0] || null,
      };

      await itemsAPI.reportFound(itemData);
      onNavigate('home'); // Or navigate to a success screen
    } catch (error) {
